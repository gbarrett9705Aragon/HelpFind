...
- [x] Updated Google Apps Script Web App backend to Version 14 to support `refer_provider`, `report_issue`, `consent`, and `endorse` actions.
...
1. **Current Operating State**: 
   ...
   * **UI updates (This Session)**: Removed the legal disclaimer/consent overlay gate upon launch. Renamed the header navigation select menu from "Provider" to "Referral". Renamed the primary card engagement button from "I'll Use" to "Call Now" and updated "Used"/"Hires" labels/data to "Called"/"Calls" across the main directory and Provider Portal metrics. Added a robust resident keyword search scanning names, categories, services, default descriptions, and custom provider stories. Built email alerts inside the Apps Script backend to notify the Admin when new entries are added to `"Referrals"` or `"ReportedIssues"`. Added a "Click to Endorse" button to service provider cards in the directory, highlighting with a thumbs-up and count. Implemented double-layer "one endorsement per IP address" protection (via client-side localStorage checks and backend SHA-256 hashed IP uniqueness validation on the new Google Sheets tab `"Endorsements"`). Updated the Google Apps Script backend to Version 14 to support and log endorsements. Updated the Details modal to show total endorsements. All changes verified green via headless Edge integration tests.
