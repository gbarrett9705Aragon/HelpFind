...
- [x] Updated Google Apps Script Web App backend to Version 15 to support `Review Count` and `Times Called` headers, lock-protected column insertions, and `refer_provider`, `report_issue`, `consent`, and `endorse` actions.
- [x] Expanded the integration test suite (`test.js`) to verify the consent overlay, private forms, background sync, rating invisibility, and provider endorsements.
...
- [x] Implemented resident-facing keyword search that scans provider names, categories, service tags, descriptions, and custom stories.
- [x] Built email alerts inside the Apps Script backend to notify the Admin when new entries are added to `"Referrals"` or `"ReportedIssues"`.
...
1. **Current Operating State**: 
   ...
   * **UI updates (This Session)**: Added a "Click to Endorse" button to service provider cards in the directory, highlighting with a thumbs-up and count. Implemented double-layer "one endorsement per IP address" protection (via client-side localStorage checks and backend SHA-256 hashed IP uniqueness validation on the new Google Sheets tab `"Endorsements"`). Updated the Google Apps Script backend to Version 15 to support thread-safe column creations and `Review Count` / `Times Called` headers. Updated the Details modal to show total endorsements. All changes verified green via headless Edge integration tests.
